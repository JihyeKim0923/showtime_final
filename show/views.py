...
def detail(request, post_id):
    ...
    return render(request, 'show/detail.html',{'post':post_detail, 'form':form, 'comments': comments,  'exhibition':exhibition})
...
def post_dislike(request, comment_id):
        # 코멘트 정보 받아옴
        comment = get_object_or_404(Comment, pk=comment_id)
        post = comment.post
        print(post.comments.all)
        ...
        # print(Comment.objects.filter(like=request.user.id).exists())
        if comment.dislikes == user: # 이 댓글에 지금 로그인한 사용자가 좋아요 누른 적 있으면
            print(aa)
